...
const ImageGrid: React.FC = () => {
    ...
    const fetchImages = async () => {
      ...
      try {
        ...
        if ('photos' in response) {
          ...
          console.log("images", response.photos)
        } else {
  ...
  const handleImageLoad = (event: any) => {
    ...
    // console.log("Image size:", event.naturalWidth, event.naturalHeight);
  };
    ...
    <div className="kd-editor-panel">
      ...
      <div className='grid grid-cols-2 gap-2 mb-4'>
        <div className='w-full'>
          <select className="kd-form-input" onChange={(e) => setOrientation(e.target.value)}>
          ...
          images.map((image, index) => (
            <div key={index} className="relative image-box-wrapper">
              <Image
                ...
                onClick={() => createclpis(image.src.original, image.width, image.height)}
