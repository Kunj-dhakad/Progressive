...
//     const url = "https://example.com/image.jpg"; // your image URL
//     const base64 = await compressImageFromUrl(url);
//     console.log("Compressed Image Base64:", base64);
...
//             className={`timeline-clip-style flex items-center rounded-lg overflow-hidden ${active_border}`}
//             style={{
//               backgroundImage: `url(${compressImage()})`,
...
const ExampleGrid: React.FC<boxCountProps> = ({ boxCount = 0, unit = 0 }) => {
  ...
  useEffect(() => {
    const generatedLayout = Allclips.map((clip) => ({
      i: clip.id,
      x: (clip.properties.start / unit),
      y: clip.properties.zindex,
      w: (clip.properties.duration / unit),
    ...
    // Calculate total duration (same as your code)
    const uniqueIntervals: { start: number; end: number }[] = [];
  ...
  // Compress images when Allclips or their image URLs change
  useEffect(() => {
  ...
  const handleSplitClip = () => {
    const Allclip = Allclips.find((clip) => clip.id === activeClip);
    ...
    if (
      ...
      const firstClip = {
        ...
        properties: {
          ...Allclip.properties,
          duration: splitTime,
          TrimEnd: Allclip.properties.TrimStart + splitTime,
        },
      };
